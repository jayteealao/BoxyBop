/**
 * POST /codegen - Generate UI package from ir.json using Claude Opus.
 *
 * Input:
 * - runs/<run_id>/ir.json
 * - locked_tokens.json + tokens.css
 * - DESIGN_SYSTEM.md (from style analysis)
 * - optional crop previews
 *
 * Output:
 * - packages/ui-<setSlug>/
 *   - src/tokens/ (must match locked tokens exactly)
 *   - src/components/ (shadcn style)
